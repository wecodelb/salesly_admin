...
/**
 * The standing lists: the customer book, who owes what, the catalogue, the
 * invoice book.
 *
 * Composed in the browser from the same reads the screens use rather than
 * from report endpoints, so a figure here can never disagree with the page it
 * was run from. The document is on screen as it will print, so the PDF is the
 * page itself.
 */
      ...
      <div className="mb-6 flex flex-col gap-4 rounded-[var(--radius-card)] bg-[var(--bg-surface)] p-4 shadow-[var(--shadow-card)]">
        ...
        <div className="flex flex-wrap items-end gap-4 border-t border-[var(--border-default)] pt-4">
          ...
          <div className="ml-auto">
            <ExportButton pdf="screen" disabled={loading || failed} build={() => doc} />
          </div>
        </div>
      </div>
